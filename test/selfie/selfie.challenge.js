...
describe("[Challenge] Selfie", function () {
  ...
  it("Execution", async function () {
    const playerContract = await ethers
      .getContractFactory("SelfieAttacker", player)
    ...
    const actionId = await playerContract.connect(player).actionId();

    // confirm that aciton is queued
    const action = await governance.connect(player).actions(actionId);
